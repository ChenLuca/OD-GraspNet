...
class Generative_ODC_Shuffle_1_Bypass_V2_OSA_Depth_3(GraspModel):

    def __init__(self, input_channels=4, output_channels=1, channel_size=256, dropout=False, prob=0.0):
        super(Generative_ODC_Shuffle_1_Bypass_V2_OSA_Depth_3, self).__init__()

        print("Generative_ODC_Shuffle_1_Bypass_V2_OSA_Depth_3")
        ...
        self.concat_1_size = self.conv_out_size + self.trans_conv_kernal[3]
        ...
        self.cbam1 = CBAM(self.trans_conv_kernal[0], 8)
        ...
        self.cbam2 = CBAM(self.trans_conv_kernal[1], 8)
        ...
        self.cbam3 = CBAM(self.trans_conv_kernal[2], 8)
        ...
        # self.conv5 = nn.ConvTranspose2d(self.concat_2_size, channel_size, kernel_size=4, stride=2, padding=2,
        #                                 output_padding=1)
        # self.bn5 = nn.BatchNorm2d(channel_size, 8)

        # self.conv6 = nn.ConvTranspose2d(channel_size, channel_size, kernel_size=9, stride=1, padding=3)

        self.conv4 = nn.Conv2d(self.concat_1_size, channel_size*4, kernel_size=3, stride=1, padding=1)
        self.sf1 = nn.PixelShuffle(self.shuffle_factor[1])
        self.bn4 = nn.BatchNorm2d(self.concat_2_size)

        self.cbam6 = CBAM(self.concat_2_size)

        self.conv5 = nn.Conv2d(self.concat_2_size, channel_size*2, kernel_size=3, stride=1, padding=1)
