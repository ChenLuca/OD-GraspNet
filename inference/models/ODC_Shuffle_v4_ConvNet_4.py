...
class Generative_ODC_Shuffle_v4_4(GraspModel):

    def __init__(self, input_channels=4, output_channels=1, channel_size=256, dropout=False, prob=0.0):
        super(Generative_ODC_Shuffle_v4_4, self).__init__()

        print("Generative_ODC_Shuffle_v4_4")
        ...
        self.conv3 = nn.Conv2d(channel_size, channel_size*2, kernel_size=3, stride=1, padding=1)
        self.conv4 = nn.Conv2d(channel_size*2, channel_size*2, kernel_size=3, stride=1, padding=1)

        self.maxpool2 = nn.MaxPool2d(2, stride=2)
        self.bn2 = nn.BatchNorm2d(channel_size*2)

        self.conv5 = nn.Conv2d(channel_size*2, channel_size * 4, kernel_size=3, stride=1, padding=1)
        ...
        self.conv6 = nn.Conv2d(self.trans_conv_kernal[0], channel_size*4, kernel_size=3, stride=1, padding=1)
        self.sf1 = nn.PixelShuffle(self.shuffle_factor[1])
        self.bn4 = nn.BatchNorm2d(32)

        self.conv7 = nn.Conv2d(32, channel_size*2, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x_in):
        ...
        x = F.relu(self.bn1(self.maxpool1(self.conv2(F.relu(self.conv1(x_in))))))
        x = F.relu(self.bn2(self.maxpool2(self.conv4(F.relu(self.conv3(x))))))
        x = F.relu(self.bn3(self.conv5(x)))

        x = self.trans1(self.block1(x))
        x = self.cbam1(x)

        x = F.relu(self.bn4(self.sf1(self.conv6(x))))
        x = F.relu(self.bn5(self.sf2(self.conv7(x))))
