...
class Generative_ODC_Shuffle_v2_4_mish(GraspModel):

    def __init__(self, input_channels=4, output_channels=1, channel_size=256, dropout=False, prob=0.0):
        super(Generative_ODC_Shuffle_v2_4_mish, self).__init__()

        print("Generative_ODC_Shuffle_v2_4_mish")
    ...
    def forward(self, x_in):
        ...
        x = self.mish1(self.bn1(self.maxpool1(self.conv1(x_in))))
        x = self.mish2(self.bn2(self.maxpool2(self.conv2(x))))
        x = self.mish3(self.bn3(self.conv3(x)))

        x = self.trans1(self.block1(x))
        x = self.cbam1(x)

        x = self.mish4(self.bn4(self.sf1(self.conv4(x))))
        x = self.mish5(self.bn5(self.sf2(self.conv5(x))))
