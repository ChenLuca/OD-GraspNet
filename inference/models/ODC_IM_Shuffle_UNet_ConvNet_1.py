...
class Generative_ODC_IM_Shuffle_UNet_1(GraspModel):

    def __init__(self, input_channels=4, output_channels=1, channel_size=256, dropout=False, prob=0.0):
        super(Generative_ODC_IM_Shuffle_UNet_1, self).__init__()

        print("Generative_ODC_IM_Shuffle_UNet_1")

        self.shuffle_factor = [2, 2, 2, 2]
        
        self.osa_depth = 5
        self.osa_conv_kernal = [32, 40, 48, 56]
        self.trans_conv_kernal = [256, 256, 256, 256]
        ...
        self.conv5 = nn.Conv2d(self.trans_conv_kernal[3], channel_size*8, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x_in):
        ...
        x = self.trans1(self.block1(x_4_down))

        x = x + x_4_down
        
        x_cbam1 = self.cbam1(x)        
        
        x = self.trans2(self.block2(x_cbam1))

        x = x + x_cbam1
        
        x_cbam2 = self.cbam2(x)        
        
        x = self.trans3(self.block3(x_cbam2))

        x = x + x_cbam2
        
        x_cbam3 = self.cbam3(x)        

        x = self.trans4(self.block4(x_cbam3))
