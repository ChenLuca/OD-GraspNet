...
class Generative_ODC_Shuffle_v3_4(GraspModel):

    def __init__(self, input_channels=4, output_channels=1, channel_size=256, dropout=False, prob=0.0):
        super(Generative_ODC_Shuffle_v3_4, self).__init__()

        print("Generative_ODC_Shuffle_v3_4")

        self.shuffle_factor = [2, 2]


        self.conv1 = nn.Conv2d(input_channels, channel_size, kernel_size=3, stride=1, padding=1)
        self.usf1 = nn.PixelUnshuffle(self.shuffle_factor[0])
        self.bn1 = nn.BatchNorm2d(channel_size*4)

        self.conv2 = nn.Conv2d(channel_size*4, channel_size*4, kernel_size=3, stride=1, padding=1)
        self.usf2 = nn.PixelUnshuffle(self.shuffle_factor[1])
        self.bn2 = nn.BatchNorm2d(channel_size * 16)

        self.conv3 = nn.Conv2d(channel_size * 16, channel_size * 16, kernel_size=3, stride=1, padding=1)
        self.bn3 = nn.BatchNorm2d(channel_size * 16)


        
        # self.usf1 = nn.PixelUnshuffle(self.shuffle_factor[0])
        # self.bn1 = nn.BatchNorm2d(pow(self.shuffle_factor[0], 2))

        # self.usf2 = nn.PixelUnshuffle(self.shuffle_factor[1])0
        # self.bn2 = nn.BatchNorm2d(pow(self.shuffle_factor[0], 2) * pow(self.shuffle_factor[1], 2))

        # self.PixelUnshuffle_out_size = 16
        ...
        self.osa_reduction = 1.0
         ...
         # 1st block
        self.block1 = OSABlock(self.osa_depth, channel_size * 16, self.osa_conv_kernal[0], OSAModule, self.osa_drop_rate)
        ...
        # self.conv4 = nn.ConvTranspose2d(self.trans_conv_kernal[0], channel_size * 2, kernel_size=4, stride=2, padding=1,
        #                                 output_padding=1)
        ...
        self.conv4 = nn.Conv2d(self.trans_conv_kernal[0], channel_size*16, kernel_size=3, stride=1, padding=1)
        self.sf1 = nn.PixelShuffle(self.shuffle_factor[1])
        self.bn4 = nn.BatchNorm2d(32)

        self.conv5 = nn.Conv2d(32, channel_size*4, kernel_size=3, stride=1, padding=1)
        self.sf2 = nn.PixelShuffle(self.shuffle_factor[0])
        self.bn5 = nn.BatchNorm2d(8)

        self.pos_output = nn.Conv2d(in_channels=8, out_channels=output_channels, kernel_size=3, padding=1)
        self.cos_output = nn.Conv2d(in_channels=8, out_channels=output_channels, kernel_size=3, padding=1)
        self.sin_output = nn.Conv2d(in_channels=8, out_channels=output_channels, kernel_size=3, padding=1)
        self.width_output = nn.Conv2d(in_channels=8, out_channels=output_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x_in):
        ...
        x = F.relu(self.bn1(self.usf1(self.conv1(x_in))))
        x = F.relu(self.bn2(self.usf1(self.conv2(x))))
