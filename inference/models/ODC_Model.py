...
class OSAModule(nn.Module):
    def __init__(self, in_planes, out_planes, dropRate=0.0):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=1,
                               padding=1, bias=False)
        self.droprate = dropRate
...
class OSABlock(nn.Module):
    def __init__(self, nb_layers, in_planes, growth_rate, block, dropRate=0.0):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.conv = nn.Conv2d(nb_layers*growth_rate, growth_rate, kernel_size=1, stride=1,
                               padding=1, bias=False)
...
class OSADenseNet_CBAM(nn.Module):
    def __init__(self, depth, growth_rate=32,
                 reduction=0.5, bottleneck=True, dropRate=0.0):
        super(OSADenseNet_CBAM, self).__init__()
        ...
        self.cbam1 = CBAM(in_planes*n, 8)

        self.trans1 = TransitionBlock(in_planes*n, int(math.floor(in_planes*reduction)), dropRate=dropRate)
        in_planes = int(math.floor(in_planes*reduction))

        # 2nd block
        self.block2 = OSABlock(n, in_planes, growth_rate, block, dropRate)
        in_planes = int(growth_rate)
        self.cbam2 = CBAM(in_planes*n, 8)

        self.trans2 = TransitionBlock(in_planes*n, int(math.floor(in_planes*reduction)), dropRate=dropRate)
        in_planes = int(math.floor(in_planes*reduction))

        # 3rd block
        self.block3 = OSABlock(n, in_planes, growth_rate, block, dropRate)
        in_planes = int(growth_rate)
        self.cbam3 = CBAM(growth_rate*n, 8)
    ...
    def forward(self, x):
        out = self.conv1(x)
        out = self.cbam1(self.block1(out))
        out = self.trans1(out)
        out = self.cbam2(self.block2(out))
        out = self.trans2(out)
        out = self.block3(out)
        out = self.cbam3(out)
